...
pub struct TypedBytesWriter(pub Vec<u8>);
...
impl<'a> BytesReader<'a> {
    ...
    pub fn read_u256(&mut self) -> Result<U256> {
        let value =
            U256::from_be_bytes::<32>((&self.slice[(self.pos)..(self.pos + 32)]).try_into()?);
        self.pos = self.pos + 32;
        Ok(value)
    }
    pub fn read_string(&mut self, _len: usize) -> Result<String> {
        let value = unsafe {
            str_from_null_terminated_utf8((&self.slice[(self.pos)..]).as_ptr() as *const u8)?
        };
